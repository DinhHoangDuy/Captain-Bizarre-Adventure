    ...
    */
    ...
    #region Captain's Skill Set Attributes
        ...
        [Header("Captain's Basic Attributes")]
            ...
            [SerializeField] private int startSP = 20;
            [SerializeField] private float SPRegenRate = 2f;
            [SerializeField] private int maxSP = 90;
        ...
        [Header("Captain's Ultimate Attributes")]
            ...
            [SerializeField] private int requiredSP = 50;
            //Stack system
    ...
    //Current Status
    private float currentBasicDamage;
    ...
    private void Start()    
    {
        ...
        currentBasicDamage = basicDamage;
        currentSP = startSP;
        //Stack system
    ...
    private void Update()
    {
        ...
        //Stack system: Generate the stack
        if(currentUltStack < maxUltStack)
        {
            if(currentStackReGenCooldown <= 0)
            {
                currentUltStack++;
                Debug.Log("Current Stack of Ultimate has been generated by 1");
    ...
    {   
        ...
        basicAttackDamage = 10 + (currentBasicDamage * basicAttackMultiplier / 100);
        //Play the animation
    ...
    private void UltimateAttack()
    {
        ...
        ultimateDamage = ultimateBaseDamage + (currentBasicDamage * ultimateDamageMultiplier / 100);
        ShootEnergyWave();
    }
    //Passive
    private void BasicAttackPassiveTrigger()
    {
        if(UnityEngine.Random.Range(0, 100) <= passiveTriggerChance)
        {
            if(!isPassiveActive)
            {
                ...
                StartCoroutine(PassiveDuration());
            }
            else
            {
                //Reset the passive duration
                StopCoroutine(PassiveDuration());
                StartCoroutine(PassiveDuration());
            }
        }
    }
    private IEnumerator PassiveDuration()
    {
        if(!isPassiveActive)
        {
            isPassiveActive = true;
